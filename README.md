...
A. 첫 실행은 반드시 `ALIEXPRESS_HEADLESS=false` 로 수행해 수동 캡차 통과 →
`storage_state` 저장. 이후 실행부터는 자동입니다.
`ALIEXPRESS_LOGIN_WAIT=180` (초)을 늘리면 수동 대기 여유가 늘어납니다.
...
A. 네이버 로그인 키 변경에 따른 일시 오류일 수 있습니다. `common/auth.py`
의 `naver_login()` 이 키를 매 호출 시 새로 가져오므로 재시도하면 보통
해결됩니다. 지속적으로 실패하면 Chrome 프로필 쿠키 폴백 (`NAVER_CHROME_PROFILE`)
을 활성화하세요.
