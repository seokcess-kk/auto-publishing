...
> **어필리에이트 의무 고지** — 문구는 `common/affiliate_notice.py` 한 곳에서
> 관리되며 모든 채널(티스토리·WP·네이버 블로그/카페·GitHub Pages·Threads·
> Pinterest)의 **본문 최상단**에 자동 삽입됩니다. 쿠팡 파트너스 심사는
> "활동 게시물 최상단 혹은 제목" 표기와 조건부 표현("받을 수 있음") 금지를
> 요구합니다 — 하단 푸터 표기는 2026-08-10 최종 승인 반려 사유였습니다.
> 임의로 제거하거나 위치를 옮기면 파트너스 자격이 반려됩니다.
> `.env` 에 `AFFILIATE_NOTICE_IN_TITLE=true` 를 넣으면 제목에도 `[광고]`
> 접두어가 붙습니다(기본 off).
